...
\baheem\ is also is highly parallelise-able, and requires only three
additions and a single \gls{xor}.
...
\begin{description}[itemsep=0em]
    ...
    \item[$\mathbf{p}_b = \random(128), \mathbf{q}_b = \random(128)$:]  A
        pair of uniformly distributed random bits.  This is generated
        dynamically by the implementation, transparently from the user, for
        every block.
...
It follows from Shannon's perfect secrecy proof of the \gls{otp}
...
As shown in \cref{tbl_3bit_add}, the total number of horizontal and
...
\begin{note}
    This paper does not claim that \baheem\ has perfect secrecy, but rather
    claims that it has a $\entropy(\mathbf{k})$-bit security. Shannon's
    perfect secrecy proof is cited only for its relevance in proving
    \baheem's $\entropy(\mathbf{k})$-bit security as the \gls{otp} is
    nonetheless a similar \gls{xor} cryptosystem.
\end{note}
...
\subsection{Alyal}
Alyal is an early single-threaded implementation to demonstrate \baheem's
...
\Cref{tbl_benchmark} shows that, while the early \baheem\ prototype, Alyal,
