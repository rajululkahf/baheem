...
$\mathtt{len} \times 128$ bits long cleartext message $\mathbf{m}$.

However, since $128$-bit wide CPU instructions are not common, this code
does the operations in $64$-bit basis, each time with a different $64$ bit
part of the key.

The encryption happens in-place, so the caller does not have to allocate
separate memory for the output.
