...
requires only three additions (one per-session, two per-block) and one
\gls{xor} in order to encrypt or decrypt, and is also highly
parallelise-able.

Despite \baheem's $1$-bit overhead per cleartext bit, its early prototype,
