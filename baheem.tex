...
\begin{description}
    \item[Note:] \baheem\ does not aim at achieving perfect secrecy, as
        ...
        $\min[\entropy(\mathbf{m}_b), \entropy(\mathbf{k})] \le 128$
        entropy bits.  Unlike the \gls{otp}, this is \emph{practically}
        secure and only requires a small $128$-bit pre-shared key.
...
In \cref{lst_baheem_c_enc}, the caller is expected to initialise a $128$
bits key $\mathbf{k}$, a pair of random pads, $\mathbf{p}$ and
$\mathbf{q}$, each of which is $\mathtt{len}$ long, in order to encrypt a
$\mathtt{len}$ long cleartext message $\mathbf{m}$. The encryption happens
in-place, so the caller does not have to allocate separate memory for the
output. Since $128$-bit wide CPU instructions are not common, this code
operates in $64$-bit basis, each time with a different $64$-bit part of the
key.
...
\begin{table}[tbh]
\hrule
\begin{lstlisting}[language=C, caption=Encryption function example,
                   label=lst_baheem_c_enc]
void baheem_enc(
...
Likewise, \cref{lst_baheem_c_dec} shows an example implementing the
corresponding in-place decryption function.

\begin{table}[tbh]
\hrule
\begin{lstlisting}[language=C, caption=Decryption funciton example,
                   label=lst_baheem_c_dec]
...
published article.  Implementation code is available in the following
repository:

\url{https://codeberg.org/rajululkahf/alyal}
